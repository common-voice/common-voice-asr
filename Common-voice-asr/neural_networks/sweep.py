...
def sweep_train():
    try:
        with wandb.init():
            ...
            check_data = False
            train(check_data, config.full_mini, config.model_type, config.epochs, config.learning_rate, logdir, config.batch_size, 
                config.hidden_dimension, lm_weight=config.lm_weight, word_score=config.word_score)
